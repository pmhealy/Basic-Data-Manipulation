...
std_company <- c("philips", "akzo", "vanhouten","unilever")
library(stringdist)
...
colnames(company_dummy)[1] <- "company_philips"
colnames(company_dummy)[2] <- "company_akzo"
colnames(company_dummy)[3] <- "company_vanhouten"
colnames(company_dummy)[4] <- "company_unilever"

prod_dummy<-dummy(refine_df$Product_Category)
colnames(prod_dummy)[1] <- "product_smartphone"
colnames(prod_dummy)[2] <- "product_tv"
colnames(prod_dummy)[3] <- "product_laptop"
colnames(prod_dummy)[4] <- "product_tablet"
